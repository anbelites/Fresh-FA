...
def _speech_islands(
    ...
    *,
    ...
    min_speech_sec = _float_env("FA_ASR_SPEECH_MIN_ISLAND_SEC", 0.6)
    merge_gap_sec = _float_env("FA_ASR_SPEECH_MERGE_GAP_SEC", 0.9)
    pad_sec = _float_env("FA_ASR_SPEECH_PAD_SEC", 0.45)
    max_island_sec = _float_env("FA_ASR_SPEECH_MAX_ISLAND_SEC", _windowed_window_sec())
    threshold_ratio = _float_env("FA_ASR_SPEECH_RMS_THRESHOLD_RATIO", 0.18)
    ...
    threshold = max(noise * 2.5, peak * threshold_ratio, 1e-5)
    raw: list[list[float]] = []
    ...
    if not islands:
        islands = [{"index": 0.0, "start": 0.0, "end": round(duration_sec, 3), "speech_ratio": 1.0}]
        ...
        if primary_profile.name in ("windowed_strict", "windowed_enhanced"):
            ...
            if _candidate_router_enabled():
                ...
                try:
                    ...
                    asr_profile_history.append(_asr_profile_metadata(router_profile, router_kw))
                    windowed_report = {
                        **router_report,
