...
def _preview_word_timestamp_alignment(seg_list: list[Any]) -> dict[str, Any]:
    words_total = 0
    words_with_timestamps = 0
    ...
    for seg in seg_list:
        ...
        for word in words:
            ...
            if getattr(word, "start", None) is not None and getattr(word, "end", None) is not None:
                words_with_timestamps += 1
    words_pct = words_with_timestamps / max(1, words_total) * 100.0
    status = "pass" if words_pct >= 95.0 and words_total > 0 else "warn"
    return {
        "status": status,
...
def _alignment_better_or_equal(candidate: dict[str, Any], baseline: dict[str, Any]) -> bool:
    try:
        cand_words = float(candidate.get("words_with_timestamps_pct") or 0.0)
        base_words = float(baseline.get("words_with_timestamps_pct") or 0.0)
    except (TypeError, ValueError):
        return False
    if candidate.get("status") != "pass" and baseline.get("status") == "pass":
        return False
    return cand_words >= base_words - _float_env("FA_ASR_ROUTER_MAX_WORD_TS_DROP", 1.0)
        ...
        if primary_profile.name in ("windowed_strict", "windowed_enhanced"):
            ...
            if _candidate_router_enabled():
                ...
                try:
                    ...
                    router_alignment = _preview_word_timestamp_alignment(router_segments)
                    base_quality = _preview_transcript_for_quality(
                        seg_list,
                        duration_sec=audio_duration_sec,
                        expected_speaker_count=expected_speaker_count,
                    )
                    base_alignment = _preview_word_timestamp_alignment(seg_list)
                    ...
                    windowed_report = {
                        **router_report,
                        ...
                        "balanced_alignment": base_alignment,
                        "windowed_alignment": router_alignment,
                    }
                    if _candidate_quality_better_or_equal(
                        router_quality, base_quality
                    ) and _alignment_better_or_equal(router_alignment, base_alignment):
