...
def _asr_profile_name() -> str:
    ...
    if raw in ("accurate", "rescue", "balanced", "windowed"):
        return raw
    return "balanced"
...
def _asr_profile(profile_name: str | None = None) -> AsrProfile:
    ...
    if name == "windowed":
        base.update(
            {
                "beam_size": _int_env("FA_ASR_WINDOWED_BEAM_SIZE", 3),
                "best_of": _int_env("FA_ASR_WINDOWED_BEST_OF", 3),
                "temperature": [0.0, 0.2],
                "vad_filter": os.environ.get("FA_ASR_WINDOWED_VAD_FILTER", "0").strip().lower()
                in ("1", "true", "yes", "on"),
                "compression_ratio_threshold": _float_env(
                    "FA_ASR_WINDOWED_COMPRESSION_RATIO_THRESHOLD", 2.8
                ),
                "log_prob_threshold": _float_env("FA_ASR_WINDOWED_LOG_PROB_THRESHOLD", -1.4),
                "no_speech_threshold": _float_env("FA_ASR_WINDOWED_NO_SPEECH_THRESHOLD", 0.8),
                "condition_on_previous_text": False,
            }
        )
        return AsrProfile(name="windowed", options=base, description="enhanced overlapping window pass")
...
def _asr_voice_filter(profile_name: str) -> str:
    if profile_name == "windowed":
        env_name = "FA_ASR_WINDOWED_AUDIO_FILTER"
    else:
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="fa_asr_windowed_") as tmp:
        ...
        for idx, window in enumerate(windows):
            ...
            _write_asr_clip(wav, clip, start, end, profile_name="windowed")
            ...
            for seg in seg_list:
                ...
                candidate["asr_window"] = {"start": round(start, 3), "end": round(end, 3)}
    ...
    report = {
        ...
        "windows": attempts,
        "merge": merge_report,
        "audio_filter": _asr_voice_filter("windowed"),
    }
...
def transcribe_video_to_structure(
    ...
    with tempfile.TemporaryDirectory(prefix="fa_transcribe_") as tmp:
        ...
        if primary_profile.name == "windowed":
            try:
                ...
                asr_failures.append({"stage": "windowed", "error": str(exc)})
